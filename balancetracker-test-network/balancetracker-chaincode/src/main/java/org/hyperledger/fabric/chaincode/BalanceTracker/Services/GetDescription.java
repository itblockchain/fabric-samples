 ...
 */
public class GetDescription extends ServiceBase implements ServiceInterface  {

    /**
     * Logger initialization
     */
    final static Logger logger = Logger.getLogger(GetDescription.class.getName());
    ...
    /**
     * Getting the project description master service
    ...
    public Object callService(List<String> args, ChaincodeStub _stub) throws BalanceTrackerException {
        logger.entering(this.getClass().getSimpleName(), "getDescription()");

        // check basic config and auth
        super.callService(args, _stub);

        if (args.size() != 0)
            throw new GetDescription.ParametersIncorrectNumberException("Incorrect number of arguments, expecting exactly 0");

        String description = BalanceTrackerBase.getDescription();

        logger.exiting(this.getClass().getSimpleName(), "getDescription()");
